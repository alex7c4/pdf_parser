# The task
Create an async document processing application.
Users can upload one or more PDF files via a website that you’ll create.
After some time, a per-page content and summarization of these files (one summary per each uploaded file) is displayed on the page.
Application accepts any PDF and parses it to a text (or markdown if possible) using one of the following methods
(allow user to choose between those methods):
- PyPDF - this won’t extract markdown, only text, but that’s fine.
- Google Gemini 2.0 Flash (use its free tier to provision API key) - you can use it for parsing of a PDF file to markdown

The backend should take the uploaded PDF, the desired parser from above, then parse the contents,
and finally output markdown-formatted text and a summary of that text.
Tech stack to use:
